	...
	SubShader {
		...
		Pass {
			...
			// As explained in the video, this multiplies the uv coordinates to create more strands because it generates more seeds
				...
				// Removing the check for `localDistanceFromCenter` to prevent rounding of squares.
				// Pixels will no longer be discarded based on their distance from the center of the UV.
				// Now, it only discards pixels based on shell index and thickness logic.

				// Check if the pixel is outside the thickness for the given shell index.
				// We no longer base the discarding on the distance from the center.
				int outsideThickness = _ShellIndex > 0 && rand < h;

				// Discard pixels only based on shell thickness and not localDistanceFromCenter
